...
class TestMiSdofToleranceZones(unittest.TestCase):
    ...
    def test_circular_zone(self):
        ...
        self.assertAlmostEqual(dist_func[1], np.sqrt(0.5**2 + 0.5**2))
    ...
    def test_cylindrical_zone(self):
        X = np.array([[0, 0, 0, 0], [0.5, 0.5, 0.5, 0.5], [1, 1, 1, 1]])
        t = 1
        L = 2
    ...
    def test_two_parallel_planes(self):
        X = np.array([[0, 0, 0], [0.5, 0.1, 0.1], [1, 1, 1]])
        t = 1
        Lx, Ly = 2, 2
...
class TestUtilityFunctions(unittest.TestCase):
    ...
    def test_points_within_bounds(self):
        sample = np.array([[0, 0], [0.5, 0.5], [1, 1]])
        bounds = np.array([[-0.5, -0.5], [0.5, 0.5]])
        expected_valid = np.array([0, 1, 0])
