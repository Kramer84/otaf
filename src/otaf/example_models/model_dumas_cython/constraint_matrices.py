...
def make_assembly_model(
    L: np.ndarray,
    Nd: int = 8,parameters
    strategy: LinearizationStrategy = LinearizationStrategy.CIRCUMSCRIBED,
):
    """
    Build the constraint matrices and wrap them in a
    ``SystemOfConstraintsAssemblyModel`` instance.

    Parameters
    ----------
    L        : array-like, shape (11,) – nominal lengths
    Nd       : int  – polygon facets per constraint (default 8)
    strategy : LinearizationStrategy  (default CIRCUMSCRIBED)

    Returns
    -------
    SystemOfConstraintsAssemblyModel
        Ready for ``model(deviation_array)`` calls.

    Example
    -------
    >>> model = make_assembly_model(L, Nd=16, strategy=LinearizationStrategy.MEAN)
    >>> C, A_ub, B_ub, A_eq, B_eq, bounds = model(deviation_samples)
    >>> result = linprog(C, A_ub=A_ub[:, :, i], b_ub=B_ub[:, i], ...)
    """
    # Lazy import so the builder is usable standalone without the class available.
    from tolerance_analysis import SystemOfConstraintsAssemblyModel  # adjust import path

    matrices = build_constraint_matrices(L, Nd=Nd, strategy=strategy)
    return SystemOfConstraintsAssemblyModel(matrices=matrices)
...
def transform_distribution(coeffs: list[float], mus: list[float], sigmas: list[float]) -> tuple[float, float]:
    """
    Calculates the equivalent normal distribution parameters (mu, sigma)
    for a linear combination of independent random variables.

    Parameters:
        coeffs: The linear multipliers (e.g., 1/L[2], -1/L[2])
        mus: The means of the corresponding variables
        sigmas: The standard deviations of the corresponding variables

    Returns:
        mu_rot, sigma_rot
    """
    mu_rot = sum(c * mu for c, mu in zip(coeffs, mus))
    var_rot = sum((c ** 2) * (sigma ** 2) for c, sigma in zip(coeffs, sigmas))

    return mu_rot, np.sqrt(var_rot)


def convert_translations_to_rotations(L: np.ndarray, trans_params: dict) -> dict:
    """
    Maps translation distribution parameters to their rotational equivalents.

    Parameters:
        L: array-like, nominal lengths [l1, l2, ..., l11].
           Note: 0-indexed, meaning l3 is L[2], l4 is L[3], etc.
        trans_params: dict mapping variable names to their (mu, sigma) parameters.
                      e.g., {'u1b1,B': (0.0, 0.01), 'u1b1': (0.0, 0.01)}

    Returns:
        dict: Rotational variables mapped to their equivalent (mu, sigma).
    """
    rot_params = {}

    def get_p(*keys):
        """Helper to safely fetch lists of mus and sigmas for given variable names."""
        mus = [trans_params[k][0] for k in keys]
        sigmas = [trans_params[k][1] for k in keys]
        return mus, sigmas

    # -----------------------------------------------------------------------
    # 1. Goupilles (Pins) B and C
    # -----------------------------------------------------------------------

    # β1b1 = (u1b1,B - u1b1) / l3
    mus, sigmas = get_p('u1b1,B', 'u1b1')
    rot_params['beta_1b1'] = transform_distribution([1/L[2], -1/L[2]], mus, sigmas)

    # α1b1 = (v1b1 - v1b1,B) / l3
    mus, sigmas = get_p('v1b1', 'v1b1,B')
    rot_params['alpha_1b1'] = transform_distribution([1/L[2], -1/L[2]], mus, sigmas)

    # β2b2 = (-u2b2,E + u2b2) / l5
    mus, sigmas = get_p('u2b2,E', 'u2b2')
    rot_params['beta_2b2'] = transform_distribution([-1/L[4], 1/L[4]], mus, sigmas)

    # α2b2 = (-v2b2 + v2b2,E) / l5
    mus, sigmas = get_p('v2b2', 'v2b2,E')
    rot_params['alpha_2b2'] = transform_distribution([-1/L[4], 1/L[4]], mus, sigmas)

    # β1c1 = (u1c1,D - u1c1) / l4
    mus, sigmas = get_p('u1c1,D', 'u1c1')
    rot_params['beta_1c1'] = transform_distribution([1/L[3], -1/L[3]], mus, sigmas)

    # α1c1 = (v1c1 - v1c1,D) / l4
    mus, sigmas = get_p('v1c1', 'v1c1,D')
    rot_params['alpha_1c1'] = transform_distribution([1/L[3], -1/L[3]], mus, sigmas)

    # β2c2 = (-u2c2,F + u2c2) / l6
    mus, sigmas = get_p('u2c2,F', 'u2c2')
    rot_params['beta_2c2'] = transform_distribution([-1/L[5], 1/L[5]], mus, sigmas)

    # α2c2 = (-v2c2 + v2c2,F) / l6
    mus, sigmas = get_p('v2c2', 'v2c2,F')
    rot_params['alpha_2c2'] = transform_distribution([-1/L[5], 1/L[5]], mus, sigmas)


    # -----------------------------------------------------------------------
    # 2. Appuis Plans (Planes) A1 and A2
    # -----------------------------------------------------------------------
    # Note: Replace `c1`, `c2`, `c3` with the exact algebraic fractions derived
    # from the thesis formulas for the planes.

    # Example template for α1a1 assuming 3 translation points:
    # mus, sigmas = get_p('w1a1,H', 'w1a1', 'w1a1,C')
    # c1 = L[0] / (...)          # coefficient for w1a1,H
    # c2 = (L[9] - L[0]) / (...) # coefficient for w1a1
    # c3 = -L[9] / (...)         # coefficient for w1a1,C
    # rot_params['alpha_1a1'] = transform_distribution([c1, c2, c3], mus, sigmas)
